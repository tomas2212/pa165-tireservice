...
<p>
<strong><s:label for="o7"><f:message key="tires.to.select"/></s:label></strong>
<table>
    <thead>
...
<p>
<strong><s:label for="o7"><f:message key="selected.tires"/></s:label></strong>
<table class="aui">
    <thead>
        <td><f:message key="field.manufacturer"/></td>
        <td><f:message key="field.amount"/></td>
    ...
    <tbody>
        <c:forEach items="${order.tires}" var="tire">
            <tr>
                ...
                <td><s:submit name="removeTire"><s:param name="tireType.id" value="${tire.tireType.id}"/><f:message key="action.remove" /></s:submit></td>
            </tr>
        </c:forEach>
    </tbody>
</table></p>
